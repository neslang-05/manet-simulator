...
NS_LOG_COMPONENT_DEFINE("LabDsr");
...
int main(int argc, char *argv[])
{
    ...
    std::string animFile    = "dsr-animation.xml";
    ...
    NS_LOG_INFO("[DSR] Nodes=" << nNodes << " SimTime=" << simTime
                << " Area=" << areaX << "x" << areaY);
    ...
    // ── DSR Routing ────────────────────────────────────────────────
    // DSR requires installing internet FIRST, then DSR on top
    ...
    {
        anim.UpdateNodeDescription(nodes.Get(i), "DSR-" + std::to_string(i));
        // Orange colour to identify DSR nodes
        anim.UpdateNodeColor(nodes.Get(i), 255, 140, 0);
    }
    ...
    std::cout << "\n[DSR] ===== Simulation Results =====" << std::endl;
    std::cout << "[DSR] TX Packets   : " << g_tx        << std::endl;
    std::cout << "[DSR] RX Packets   : " << g_rx        << std::endl;
    std::cout << "[DSR] PDR          : " << pdr         << " %" << std::endl;
    std::cout << "[DSR] Avg Throughput: "
              << (flowCount > 0 ? totalTp / flowCount : 0) << " kbps" << std::endl;
    std::cout << "[DSR] Avg Delay    : "
              << (flowCount > 0 ? totalDelay / flowCount : 0) << " ms" << std::endl;
    std::cout << "[DSR] Animation    : " << animFile    << std::endl;
    std::cout << "[DSR] ====================================\n" << std::endl;
