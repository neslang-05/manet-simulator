...
NS_LOG_COMPONENT_DEFINE("LabAodv");
...
int main(int argc, char *argv[])
{
    ...
    std::string animFile    = "aodv-animation.xml";
    ...
    NS_LOG_INFO("[AODV] Nodes=" << nNodes << " SimTime=" << simTime
                << " Area=" << areaX << "x" << areaY);
    ...
    {
        anim.UpdateNodeDescription(nodes.Get(i), "AODV-" + std::to_string(i));
        // Blue colour to identify AODV nodes
        anim.UpdateNodeColor(nodes.Get(i), 0, 102, 204);
    }
#endif
    ...
    std::cout << "\n[AODV] ===== Simulation Results =====" << std::endl;
    std::cout << "[AODV] TX Packets   : " << g_tx       << std::endl;
    std::cout << "[AODV] RX Packets   : " << g_rx       << std::endl;
    std::cout << "[AODV] PDR          : " << pdr        << " %" << std::endl;
    std::cout << "[AODV] Avg Throughput: "
              << (flowCount > 0 ? totalTp / flowCount : 0) << " kbps" << std::endl;
    std::cout << "[AODV] Avg Delay    : "
              << (flowCount > 0 ? totalDelay / flowCount : 0) << " ms" << std::endl;
    std::cout << "[AODV] Animation    : " << animFile   << std::endl;
    std::cout << "[AODV] ====================================\n" << std::endl;
